local SpinBob = {}
SpinBob.__index = SpinBob

SpinBob.BindableEvent = Instance.new("BindableEvent")
SpinBob.BindableEvent.Parent = script

SpinBob.TagName = "SpinBob"
...
Configuration for a coin "Class":
	- Model  : Required for creation. the name of the BasePart in ServerStorage.
...
-- apply defaults
for class, info in pairs(SpinBob.info) do
	apply_defaults(info, SpinBob.defaults)
end
...
function SpinBob.fromClass(class_name)
	local rinfo = SpinBob.info[class_name]
	if rinfo ~= nil then
		return setmetatable(deepcopy(rinfo), SpinBob)
	end
	print("did not find", class_name)
	return nil
end

function SpinBob:TransSave()
	local trans = {}
...
function SpinBob:TransSet(value)
	for pp, vv in pairs(self.trans) do
		pp.Transparency = vv + (1 - vv) * value
	end
end

function SpinBob:TransRestore(trans)
	for pp, vv in pairs(self.trans) do
		pp.Transparency = vv
	end
end

function SpinBob:TweenAdd(tween)
	table.insert(self.tweens, tween)
	tween:Play()
end

function SpinBob:TweensCancel()
...
function SpinBob.PartInfo(part)
	...
	local info = SpinBob.fromClass(vv.Value)
	if info == nil then
		warn(part, "Missing Info", vv.Value)
		return
	end

	for k, v in pairs(SpinBob.overrides) do
...
function SpinBob.GetInfo(class_name, overrides)
	local info = SpinBob.fromClass(class_name)
	if info == nil then
	...
	if overrides ~= nil then
		for k, v in pairs(overrides) do
			if SpinBob.overrides[k] ~= nil then
				info[k] = v
			end
		end
	end
